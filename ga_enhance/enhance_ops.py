...
# 🟢 维度改为 6 (2个色彩参数 + 3个Gamma参数 + 1个锐化权重参数)
DIM = 6
...
def decode_params(chrom) -> dict:
    """
    解码染色体。
    chrom[5] 是新增的基因，用于控制锐化图的权重 w_sharp [cite: 170-171]。
    """
    chrom = np.asarray(chrom, dtype=float).reshape(-1)
    if len(chrom) != DIM:
        raise ValueError(f"[decode_params] 维度不匹配，期望 {DIM}，实际得到 {len(chrom)}")

    g0, g1, g2, g3, g4, g5 = chrom.tolist()

    return {
        "eta1": g0,
        "eta2": g1,
        "gamma1": 0.8 + g2 * 3.0,  # 允许稍微调暗，增加搜索空间 [cite: 129]
        "gamma2": 1.0 + g3 * 2.0,
        "gamma3": 1.0 + g4 * 2.0,
        "w_sharp": g5  # 🟢 新增：锐化权重基因 [0, 1]
    }
...
def apply_enhancement(img_bgr: np.ndarray, params: dict) -> np.ndarray:
    """
    执行论文中的联合增强流程，w_sharp 现在由 GA 决定。
    """
